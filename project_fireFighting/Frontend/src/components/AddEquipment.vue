...
<template>
  <v-container fluid class="main-section main-dialog-section pa-0">
      <HeaderItem class="dialog-header" headerTitle="הוספת ציוד" hideSignout hideBack hideHome hidePlaceholder></HeaderItem>
      <v-row class="contents-row ma-0 px-3">
        <v-form class="dialog-form-content" ref="form" v-model="valid" lazy-validation>
          <div class="mt-6 form-input">
            <h3>סטטוס:<span class="text-red"> *</span></h3>
            <v-radio-group class="mt-2" 
            v-model="formData.reqp_is_new"
            :rules="[v => v !== null && v !== undefined || 'זהו שדה חובה.']" 
            inline
            required>
                <v-radio class="me-9" label="ציוד קיים" :value="0" color="red darken-3"></v-radio>
                <v-radio label="ציוד חדש" :value="1" color="red darken-3"></v-radio>
            </v-radio-group>
          </div>
          <v-text-field
                class="form-input mt-3"
                v-model="formData.reqp_id"
                density="comfortable"
                variant="outlined"
                :rules="[v => !!v || 'זהו שדה חובה.', checkDuplicateReqpId]"
                label="מספר זיהוי"
                required>
          </v-text-field>
          ...
          <v-text-field
            ...
            v-model.number="formData.reqp_pressure_test_year"
            density="comfortable"
            variant="outlined"
...
export default {
  ...
  data: () => ({
      ...
      formData: {
        ...
        reqp_details: null,
        reqp_location: '',
        reqp_belongs_cabinet: null,
        reqp_pressure_test_year: null,
        reqp_is_new: null,
  ...
  methods: {
    checkDuplicateReqpId(value) {
      return (
          !this.reportedEquipments.some((item) => item.reqp_id === value) ||
          'כבר קיים ציוד עם מספר זיהוי זה.'
      );
    },
    closeDialog() {
  ...
  async mounted()
  {
      this.formData.reqp_date = this.todayFormatted;
      this.formData.reqp_client = this.client.client_id;
